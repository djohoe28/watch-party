...
describe("computeSyncTargetTime", () => {
	it("returns the server currentTime unchanged when paused", () => {
		...
		expect(computeSyncTargetTime(media, 0, 1, 10_000)).toBe(42);
	});
	...
	it("does not correct for drift within maxDelta tolerance", () => {
		...
		expect(computeSyncTargetTime(media, 9.6, 1, 500)).toBe(10);
	});
	...
	it("adds elapsed transit time when playing and drift exceeds maxDelta", () => {
		...
		expect(computeSyncTargetTime(media, 0, 1, 3000)).toBe(13);
	});

	it("treats an undefined local currentTime as 0", () => {
		const media = { isPaused: false, currentTime: 10, lastUpdated: Timestamp.fromMillis(0) };
		expect(computeSyncTargetTime(media, undefined, 1, 3000)).toBe(13);
	});

	it("defaults `now` to Date.now() when not provided", () => {
		const media = { isPaused: false, currentTime: 5, lastUpdated: Timestamp.fromMillis(Date.now()) };
